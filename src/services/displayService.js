...
function getDisplayCurrent(db) {
  ...
  const mapRow = (row) => {
    ...
    return {
      ...
      memo: row.memo,
      practice: {
        split: settings.showSplit && (settings.showPractice || practiceDisplayMode) ? formatMs(practice?.split_ms) : null,
        goal: (settings.showPractice || practiceDisplayMode) ? formatMs(practice?.goal_ms) : null,
      },
      practiceRuns: practiceRuns.map((run) => ({
        split: settings.showSplit ? formatMs(run.split_ms) : null,
