...
function getSettings(db) {
  ...
  return {
    ...
    slidePageMs: row.slide_page_ms,
    displaySortMode: row.display_sort_mode,
    requestLogMode: row.request_log_mode,
    ...
    showDelta: toBool(row.show_delta),
...
function updateSettings(db, payload) {
  ...
  const next = {
    ...
    slide_page_ms: payload.slidePageMs ?? current.slide_page_ms,
    display_sort_mode: payload.displaySortMode ?? current.display_sort_mode,
    request_log_mode: payload.requestLogMode ?? current.request_log_mode,
    ...
    show_delta: payload.showDelta === undefined ? current.show_delta : Number(Boolean(payload.showDelta)),
  ...
  db.prepare(`
    UPDATE settings SET
      ...
      slide_page_ms = @slide_page_ms,
      display_sort_mode = @display_sort_mode,
